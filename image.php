...
<?php the_post() ?>
			...
			<div id="post-<?php the_ID() ?>" class="<?php calpress_post_class() ?>">
				...
				<div class="entry-content">
					<div class="entry-attachment"><a href="<?php echo wp_get_attachment_url($post->ID); ?>" title="<?php echo wp_specialchars( get_the_title($post->ID), 1 ) ?>" rel="attachment">
					<?php echo calpress_sizedimage(wp_get_attachment_url($post->ID), 940, get_the_title($post->ID)); ?>
					</a></div>
					<div class="caption entry-caption"><?php if ( !empty($post->post_excerpt) ) the_excerpt() ?></div>
                    <?php if (get_the_content() != ""): ?>
                        <p>Full Description:</p><?php the_content() ?>
                    <?php endif ?>
