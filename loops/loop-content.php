...
?>  
  ...
  <div id="post-<?php the_ID() ?>" class="<?php calpress_post_class() ?>">
  	...
  	?>
	<?php if($hed): ?>
  		<h2 class="entry-title"><a href="<?php the_permalink() ?>" title="<?php printf( __('Permalink to %s', 'sandbox'), the_title_attribute('echo=0') ) ?>" rel="bookmark"><?php the_title() ?></a><?php calpress_hook_loopcontent_title(); ?></h2>
	<?php endif ?>
    <?php if ($meta): ?>
